...
// AGENT_IMGUI_SYSTEM_PROMPT env var, then system_prompt.md sitting beside the
// agent_imgui sources (AGENT_IMGUI_SOURCE_DIR), then a built-in fallback.
std::string LoadSystemPrompt() {
  if (const char* env = std::getenv("AGENT_IMGUI_SYSTEM_PROMPT");
      env && *env) {
    if (std::string s = ReadFile(env); !s.empty()) return s;
  }
  const std::string root = AGENT_IMGUI_SOURCE_DIR;
  if (!root.empty()) {
    if (std::string s = ReadFile(root + "/system_prompt.md");
        !s.empty()) {
      return s;
    }
  }
